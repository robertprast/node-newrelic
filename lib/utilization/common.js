...
exports.metadataAuthToken = function tokenRequest(opts, agent, cb) {
  // Add default timeout of a second to the request
  if (typeof opts === 'string') {
    opts = url.parse(opts)
  }
  ...
  const req = http.request(opts, function awsRequest(res) {
    res.pipe(concat(respond))
    function respond(data) {
      agent.removeListener('errored', abortRequest)
      agent.removeListener('stopped', abortRequest)
      ...
      if (res.statusCode !== 200) {
        logger.debug(
          'Got %d %s from metadata auth token request %j',
          res.statusCode,
          res.statusMessage || '<n/a>',
          opts
        )
        return cb(new Error('Request for metadata auth token failed.'))
      } else if (!data) {
        logger.debug('Got no response data?')
        return cb(new Error('No response data received.'))
      }
  ...
  req.on('error', function requestError(err) {
    if (err.code === 'ECONNRESET') {
      logger.debug('Request for metadata auth token %j timed out', opts)
      return cb(err)
    }

    logger.debug('Message for metadata auth token %j: %s', opts, err.message)
    cb(err)
  })
  agent.once('errored', abortRequest)
  agent.once('stopped', abortRequest)
  ...
  function abortRequest() {
    logger.debug('Aborting request for metadata auth token at %j', opts)
    req.abort()
    agent.removeListener('errored', abortRequest)
    agent.removeListener('stopped', abortRequest)
    agent.removeListener('disconnected', abortRequest)
